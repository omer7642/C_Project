    ...
    while(IC_temp < IC)
    {
        ...
        if(current_type)
        {
            ...
            if (current_type == entry)
            {
                ...
                {
                    error_flag = 1;
                    fprintf(stdout,"assembly: entry line without arguments (line %d) \n",line_cnt_tmp);
                    continue;
                }
                ...
                {
                    error_flag =1;
                    fprintf(stdout,"assembly: non existing symbol in entry line (line %d)\n",line_cnt_tmp);
                }
        ...
        else //instraction line
        {
            ...
            else if(command <rts && command >lea ) //all the commands with one opcode argument
            {
                ...
                {
                    error_flag=1;
                    fprintf(stdout,"assembly: Using undefined symbol (line %d)\n",line_cnt_tmp);
                }
