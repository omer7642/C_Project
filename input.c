...
int get_command(char *current_line,unsigned char line_flag)
{
    ...
    {
        error_flag=1;
        fprintf(stderr,"assembly: No opcode\n");
        return ERROR_SIGN;
    }
    
    fprintf(stderr,"in command the line is : %s\n",current_line);
...
char *get_symbol(char *current_line,char *symbol_name)
{
    ...
    if(!isalpha(current_line[i]))
    {
                error_flag = TRUE;
                symbol_name = "\n\0";
                fprintf(stderr, "invalid symbol name on line %d", line_counter);
                return NULL;
    }
...
int get_address_type(char * operand)
{
    ...
    printf("started get address type with operand -  %s \n",operand);
    ...
    printf("finished get_address type \n");
    return curr_type;
